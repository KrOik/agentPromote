# Global Agent Rules
  ...
  - 明确文件职责和各目录负责
  - 使用驼峰命名而不是蛇串命名，确保命名符合文件职责
  - 不允许使用emoji和表情符号
  - 使用local git做版本管理，每次编辑后需要做提交，作者不包含claude
  - 保持清晰的工作目录结构和文件存放
  - 专注于理解和解决用户提出的问题，而不是创建指导/说明文档
  - 除非用户要求，否则不得创建额外的说明文档
  ...
  ## Windows Shell Rules
  - 环境：Windows 11 + PowerShell。存在ubuntu虚拟机WSL
